...
# Wmat.series  <- matrix(6.856589/apply(Wmat.series,2,max),
#                        dim(Wmat.series)[1],
#                        dim(Wmat.series)[2],
#                        byrow=T)*Wmat.series
...
CP.CI[,4] <- as.numeric(Y1.Post - (Wmat.Post)%*%OLS.gamma.naive)[T.Window]

CP.Beta.1 <- CP.PV.1 <- 
  CP.Beta.2 <- CP.PV.2 <- matrix(0,2,1001)

CPSPSC <- Conformal.Prediction.Data(Wmat.Pre.Input  = Wmat.Pre,
...
CPSPSC.Reg <- Conformal.Prediction.Data(Wmat.Pre.Input  = Wmat.Pre,
                                        Wmat.Post.Input = Wmat.Post[T.Window,],
                                        Y1.Pre.Input    = Y1.Pre,
                                        Y1.Post.Input   = Y1.Post[T.Window],
                                        Xmat.Pre.Input  = NULL,
...
write.csv(CP.CI,    sprintf("Conformal_Raw/Raw_Placebo_Group%s_CPCI_%0.3d.csv",           gp,BATCH),row.names=F)
write.csv(CP.Beta.1,sprintf("Conformal_Raw/Raw_Placebo_Group%s_CPBeta_SPSC_%0.3d.csv",    gp,BATCH),row.names=F)
write.csv(CP.Beta.2,sprintf("Conformal_Raw/Raw_Placebo_Group%s_CPBeta_SPSC_Reg_%0.3d.csv",gp,BATCH),row.names=F)
write.csv(CP.PV.1,  sprintf("Conformal_Raw/Raw_Placebo_Group%s_CPPV_SPSC_%0.3d.csv",      gp,BATCH),row.names=F)
write.csv(CP.PV.2,  sprintf("Conformal_Raw/Raw_Placebo_Group%s_CPPV_SPSC_Reg_%0.3d.csv",  gp,BATCH),row.names=F)
