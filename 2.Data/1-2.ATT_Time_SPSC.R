...
for(gp in 1:6){
  ...
  Wmat <- rbind(Wmat.Pre,Wmat.Post)
  Y    <- c(Y1.Pre,Y1.Post)
  gW.Pre <- matrix(0,length(Y1.Pre),N+mT)
  gW.Pre[,1:N]       <- Wmat.Pre
  gW.Pre[,N+1:mT]    <- bT
  
  m <- N+mT
  
  gW   <- rbind(gW.Pre, matrix(0,T1,m))
  
  GW <- t( sapply(1:dim(gW.Pre)[2],function(tt){
    apply( t(Wmat.Pre)*matrix(gW.Pre[,tt],N,T0,byrow=T), 1, mean)
  }) )
  
  GY <- ( sapply(1:dim(gW.Pre)[2],function(tt){
    mean(gW.Pre[,tt]*Y1.Pre)
  }) )
  
  OLS.gamma.naive <- my.inverse(A = t(GW)%*%(GW) )%*%(t(GW)%*%GY)
  ...
  OLS.Data <- cbind(rbind(gW.Pre,matrix(0,T1,m)),
                    rbind(Wmat.Pre,Wmat.Post),
                    c(rep(0,T0),rep(1,T1)),
                    c(Y1.Pre,Y1.Post))
  
  colnames(OLS.Data) <- (c(sprintf("G%0.4d",1:m),
                           sprintf("W%0.4d",1:N),
                           "A",
                           "Y"))
  ...
  GRAD <- GMM.Ft.Grad(OLS.Simple.Coef,OLS.Data)
  
  OLS.Simple.VAR.HAC <- Meat.HAC(Res.Mat = GMM.Ft(OLS.Simple.Coef, OLS.Data),
                                 GRAD = GRAD,
                                 beta.pos = 1:lengthb,
                                 bw.type="auto")
