...
for(rho.index in 1:3){
  ...
  for(constant.c.index in 1:5){
    ...
    if(Box.test(Y1.Pre)$p.value<0.01){
      mT <- round( T0^(1/3) )
      bT.Type <- "bs"
      bT <- bs(1:T0,df=(mT),Boundary.knots = c(-2,T0+2),intercept=T)
    } else { 
      mT <- 0
      bT.Type <- "No"
      bT <- NULL
    ...
    GMM.Data <- cbind(rbind(gY.Pre,matrix(0,T.Post,m+mT)),
    ...
    CP.CI <- Conformal.Prediction.Time(Wmat.Pre,
    ...
    CP.Regular.CI <- Conformal.Prediction.Time(Wmat.Pre,
                                               matrix(Wmat.Post[1,],1,N),
                                               Y1.Pre,
                                               Y1.Post[1],
                                               Xmat.Pre=NULL,
...
write.csv(RESULT,
          sprintf("Result_Raw_Time/Result_BATCH%0.4d.csv",BATCH),
          row.names=F)
