...
Psi.Ft <- function(theta,
  ...
  Psi.merge <- as.matrix( cbind(Psi.detrend,
                                ...
                                Psi.beta) )
  if(detrend){
    colnames(Psi.merge) <- c( sprintf("YDT%0.5d",1:ncol(Psi.detrend)),
                              sprintf("GDT%0.5d",1:ncol(Psi.gamma.detrend)),
                              sprintf("GYb%0.5d",1:ncol(Psi.gamma.SPSC)),
                              sprintf("Beta%0.5d",1:ncol(Psi.beta)) )
  } else {
    colnames(Psi.merge) <- c( sprintf("GYb%0.5d",1:ncol(Psi.gamma.SPSC)),
                              sprintf("Beta%0.5d",1:ncol(Psi.beta)) )
  }
